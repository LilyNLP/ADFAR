...
def pick_most_similar_words_batch(src_words, sim_mat, idx2word, ret_count=50, threshold=0.7):
    """
...
def attack(text_ls, true_label, predictor, stop_words_set, word2idx, idx2word, cos_sim,simplify, sim_predictor=None,
           import_score_threshold=-1., sim_score_threshold=0.5, sim_score_window=15, synonym_num=50,
           batch_size=32):
    # first check the prediction of the original text
    orig_probs = predictor([text_ls],simplify).squeeze()
    ...
    if true_label != orig_label:
        ...
        leave_1_probs = predictor(leave_1_texts, simplify,batch_size=batch_size)
        ...
        synonym_words, _ = pick_most_similar_words_batch(words_perturb_idx, cos_sim, idx2word, synonym_num, 0.7)
        synonyms_all = []
        ...
        for idx, synonyms in synonyms_all:
            ...
            new_probs = predictor(new_texts, simplify,batch_size=batch_size)
            ...
            if np.sum(new_probs_mask) > 0:
                ...
                if new_label_prob_min < orig_prob:
                    ...
                    num_changed += 1
            text_cache = text_prime[:]
        return ' '.join(text_prime), num_changed, orig_label, torch.argmax(predictor([text_prime],simplify)), num_queries
...
def main():
    ...
    if args.do_simplify:
        print("do simplifivation the simplify version is " + args.simplify_version)
    ...
    for idx, (text, true_label) in enumerate(tqdm(data)):
        ...
        if args.perturb_ratio > 0.:
            ...
            new_label, num_queries = random_attack(text, true_label, predictor, args.perturb_ratio, stop_words_set,
                                                    word2idx, idx2word, cos_sim, simplify, sim_predictor=use,
