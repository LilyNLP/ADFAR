...
from modeling import BertForSequenceClassificationAdv
...
class NLI_infer_BERT(nn.Module):
    def __init__(self,
        ...
        self.model = modeling.BertForSequenceClassificationAdvV2.from_pretrained(pretrained_dir, num_labels=nclasses).cuda()
...
class NLI_infer_BERT_adv_v3(nn.Module):
    def __init__(self,
                 pretrained_dir,
                 nclasses,
                 max_seq_length=128,
                 batch_size=32):
        super(NLI_infer_BERT_adv_v3, self).__init__()
        self.model = modeling.BertForSequenceClassificationAdvV3.from_pretrained(pretrained_dir, num_labels=nclasses).cuda()
    ...
    def text_pred(self, text_data,simlifier, simplify2, batch_size=32):
        ...
        for (i,((input_ids, input_mask, segment_ids), (input_ids2, input_mask2, segment_ids2))) in enumerate(zip(dataloader, dataloader2)):
            ...
            with torch.no_grad():
                logits = self.model(input_ids, input_mask,segment_ids,inference=True)
                probs1 = nn.functional.softmax(logits[0], dim=-1)
                probs2 = nn.functional.softmax(logits[1], dim=-1)
                attack_probs = logits[2]
                # print("probs1:", probs1, " probs2:", probs2, " attack_probs", attack_probs, "\n")
                probs = []
                ...
                for (ii, attack_prob) in enumerate(attack_probs):
                    if attack_prob[0] <= 0.5:
                        ...
                        logits = self.model(input_ids2[ii].unsqueeze(0), input_mask2[ii].unsqueeze(0),
                                            segment_ids2 [ii].unsqueeze(0), inference=True)
                        probs2 = nn.functional.softmax(logits[1], dim=-1)
                        probs.append(probs2.squeeze().tolist())
                        '''
                        probs.append(probs2[ii].tolist())
                        '''
                probs = torch.Tensor(probs).cuda()

                '''
                if attack_probs[0][0] <= 0.5:
                    logits = self.model(input_ids2, input_mask2, segment_ids2, inference=True)
                    probs1 = nn.functional.softmax(logits[0], dim=-1)
                    probs2 = nn.functional.softmax(logits[1], dim=-1)
                    probs = probs1
                else:
                ...
                for (ii, attack_prob) in enumerate(attack_probs):
                    if attack_prob[0] <= 0.5:
                        ...
                        orig_tokens = self.tokenizer.convert_ids_to_tokens(input_ids[ii].squeeze())
                        temp = []
                        for orig_token in orig_tokens:
                            if orig_token not in ['[PAD]', '[CLS]', '[SEP]']:
                                temp.append(orig_token)
                        orig_tokens = temp
                        orig_sent = self.tokenizer.convert_tokens_to_string(orig_tokens)
                        # print("orig_sent", orig_sent)
                        simplified_sent = simplify2(orig_sent)
                        simplified_tokens = self.tokenizer.tokenize(simplified_sent)
                        if len(simplified_tokens) > 128 - 2:
                            simplified_tokens = simplified_tokens[:(128 - 2)]
                        simplified_tokens = ["[CLS]"] + simplified_tokens + ["[SEP]"]
                        for jj in range(len(simplified_tokens), 128):
                            simplified_tokens.append('[PAD]')
                        # print("simplified_sent", simplified_sent, "simplified_tokens", simplified_tokens)
                        new_input_ids = self.tokenizer.convert_tokens_to_ids(simplified_tokens)
                        new_input_ids = torch.LongTensor(new_input_ids).cuda()
                        logits = self.model(new_input_ids.unsqueeze(0), input_mask[ii].unsqueeze(0), segment_ids[ii].unsqueeze(0), inference=True)
                        probs1 = nn.functional.softmax(logits[0], dim=-1)
                        probs.append(probs1.squeeze().tolist())
                        #print("probs1:", probs1)
                        ...
                        new_input_ids = self.tokenizer.convert_tokens_to_ids(simplified_tokens)
                        new_input_ids = torch.LongTensor(new_input_ids).cuda()
                        logits = self.model(new_input_ids.unsqueeze(0), input_mask[ii].unsqueeze(0),
                                            segment_ids[ii].unsqueeze(0), inference=True)
                        probs2 = nn.functional.softmax(logits[1], dim=-1)
                        probs.append(probs2.squeeze().tolist())
                        
                        probs.append(probs2[ii].tolist())
                        #print("probs2:", probs2[ii])
...
def attack(text_ls, true_label, predictor, stop_words_set, word2idx, idx2word, cos_sim, simplify, simplify2, sim_predictor=None,
           import_score_threshold=-1., sim_score_threshold=0.5, sim_score_window=15, synonym_num=30,
           batch_size=32):
    # first check the prediction of the original text
    orig_probs = predictor([text_ls],simplify, simplify2)[0].squeeze()
    ...
    if true_label != orig_label:
        ...
        leave_1_probs = predictor(leave_1_texts, simplify, simplify2, batch_size=batch_size)[0]
        ...
        num_changed = 0
        for _,(idx, synonyms) in enumerate(synonyms_all):
            if _ > len_text/5:
                break
            new_texts = [text_prime[:idx] + [synonym] + text_prime[min(idx + 1, len_text):] for synonym in synonyms]
            new_probs,attack_probs = predictor(new_texts, simplify, simplify2, batch_size=batch_size)

            '''
            # print(attack_probs)
            if list(attack_probs.size()) == []:
                attack_prob = attack_probs
            else:
                attack_prob = attack_probs[0]
            if attack_prob <= 0.5:
                new_probs, attack_probs = predictor(new_texts, simplify2, simplify2, batch_size=batch_size)
            '''
            ...
            # new_probs_mask *= (attack_probs.squeeze() < 0.5).data.cpu().numpy()
            ...
            if np.sum(new_probs_mask) > 0:
                ...
                if new_label_prob_min < orig_prob:
                    ...
                    num_changed += 1
            text_cache = text_prime[:]
        return ' '.join(text_prime), num_changed, orig_label, torch.argmax(predictor([text_prime],simplify, simplify2)[0]), num_queries


def random_attack(text_ls, true_label, predictor, perturb_ratio, stop_words_set, word2idx, idx2word, cos_sim, simplify,
                  sim_predictor=None, import_score_threshold=-1., sim_score_threshold=0.5, sim_score_window=15,
                  synonym_num=30, batch_size=32):
    # first check the prediction of the original text
    orig_probs = predictor([text_ls], simplify)[0].squeeze()
    orig_label = torch.argmax(orig_probs)
    orig_prob = orig_probs.max()
    if true_label != orig_label:
        return '', 0, orig_label, orig_label, 0
        ...
        for idx, synonyms in synonyms_all:
            new_texts = [text_prime[:idx] + [synonym] + text_prime[min(idx + 1, len_text):] for synonym in synonyms]
            new_probs, attack_probs = predictor(new_texts, simplify, batch_size=batch_size)
            ...
            if np.sum(new_probs_mask) > 0:
                ...
                if new_label_prob_min < orig_prob:
                    ...
                    num_changed += 1
            text_cache = text_prime[:]
        return ' '.join(text_prime), num_changed, orig_label, torch.argmax(predictor([text_prime], simplify)[0]), num_queries
...
def main():
    ...
    parser.add_argument("--simplify_version",
                        ...
                        required=True,
                        help="Which dataset to attack.")
    parser.add_argument("--simplify2_version",
                        type=str,
                        required=True,
                        help="Which dataset to attack.")
    ...
    parser.add_argument("--target_model",
                        type=str,
                        required=True,
                        choices=['wordLSTM', 'bert', 'wordCNN','bert_v3'],
    ...
    parser.add_argument("--counter_fitting_embeddings_path",
                        type=str,
                        default="TextFooler/counter-fitted-vectors.txt",
    ...
    parser.add_argument("--do_simplify",
                        action='store_true',
                        help="The output directory where the attack results will be written.")

    ## Model hyperparameters
    parser.add_argument("--simp_sim_threshold",
                        default=0.1,
                        type=float,
    ...
    if args.do_simplify:
        print("do simplification the simplify version is " + args.simplify_version)
        simplifier = Simplifier(threshold=args.simp_sim_threshold, ratio=args.ratio, syn_num =args.syn_num, most_freq_num =args.most_freq_num)
        ...
        simplify2 = simplify_dict[args.simplify2_version]
    else:
        simplify = None
    ...
    elif args.target_model == 'bert_v3':
        model = NLI_infer_BERT_adv_v3(args.target_model_path, nclasses=args.nclasses, max_seq_length=args.max_seq_length)
    ...
    for idx, (text, true_label) in enumerate(tqdm(data)):
        ...
        if args.perturb_ratio > 0.:
            ...
            new_label, num_queries = random_attack(text, true_label, predictor, args.perturb_ratio, stop_words_set,
                                                    word2idx, idx2word, cos_sim, simplify, simplify2, sim_predictor=use,
